...
tf.app.flags.DEFINE_integer("beam_size", 5, "The size of beam search. Do greedy search when set this to 1.")
tf.app.flags.DEFINE_boolean("var_extract", False,
                            "If we want value of a trained variable, set flag to True")
...
def var_extract():
    """Extract some variable value by name and then the value to file."""
    with tf.Session() as sess:
        # Load vocabularies.
        en_vocab_path = os.path.join(FLAGS.data_dir,
                                     "vocab%d.src" % FLAGS.en_vocab_size)
        fr_vocab_path = os.path.join(FLAGS.data_dir,
                                     "vocab%d.trg" % FLAGS.fr_vocab_size)
        ...
        with tf.variable_scope("embedding_attention_seq2seq") as scope:
            ...
            decoder_emb = sess.run(tf.get_variable("embedding_attention_decoder/embedding"))
            emb_file = file("../emb.trg", "wb")
            pkl.dump(decoder_emb, emb_file)
            encoder_emb = sess.run(tf.get_variable("embedding"))
            emb_file = file("../emb.src", "wb")
            pkl.dump(encoder_emb, emb_file)

def main(_):
    if FLAGS.var_extract:
        var_extract()
    elif FLAGS.decode:
        decode()
    else:
