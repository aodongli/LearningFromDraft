        ...
        with vs.variable_scope(scope or type(self).__name__):  # "GRUCell"
            with vs.variable_scope("Gates"):  # Reset gate and update gate.
                ...
                if context is None:
                    r, u = array_ops.split(1, 2, _linear2([inputs, state],
                                                         2 * self._num_units, False, weight_initializer=self._weight_initializer))
                    r, u = sigmoid(r), sigmoid(u)
                else:
                    r, u = array_ops.split(1, 2, _linear2([inputs, state, context],
                                                         2 * self._num_units, False, weight_initializer=self._weight_initializer))
                    r, u = sigmoid(r), sigmoid(u)
            with vs.variable_scope("Candidate"):
                if context is None:
                    ...
                    c = self._activation(_linear2([inputs, r * state, context],
                                                 self._num_units, False, weight_initializer=self._weight_initializer))
            new_h = u * state + (1 - u) * c
        return new_h, new_h
...
def orthogonal_initializer(shape=None, scale=1.0):
    ...
    def _initializer(shape, dtype=tf.float32):
        ...
        a = np.random.normal(0.0, 0.01, flat_shape)
